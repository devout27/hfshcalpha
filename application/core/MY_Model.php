 ...
 *              If set to TRUE tells MY_Model that the table has 'created_at','updated' (and 'deleted_at' if $this->soft_delete is set to TRUE)
 *              If given an array as parameter, it tells MY_Model, that the first element is a created_at field type, the second element is a updated field type (and the third element is a deleted_at field type)
...
class MY_Model extends CI_Model
{
    ...
    /** @var bool | array
     * Enables created_at and updated fields
     */
    ...
    public function update($data = NULL, $column_name_where = NULL, $escape = TRUE)
    {
        ...
        if($multi === FALSE)
        {
            if($this->timestamps !== FALSE)
            {
                $data[$this->_updated_field] = $this->_the_timestamp();
            }
        ...
        else
        {
            ...
            foreach($data as $row)
            {
                if($this->timestamps !== FALSE)
                {
                    $row[$this->_updated_field] = $this->_the_timestamp();
                }
    ...
    public function count_rows($where = NULL)
    {
        ...
        $number_rows = $this->_database->num_rows();
        $this->_reset_trashed();
        return $number_rows;
    }
     ...
     *
     * Sets the fields for the created_at, updated and deleted_at timestamps
     * @return bool
     */
    private function _set_timestamps()
    {
        if($this->timestamps !== FALSE)
        {
            ...
            $this->_updated_field = (is_array($this->timestamps) && isset($this->timestamps[1])) ? $this->timestamps[1] : 'updated';

...
class Bank extends CI_Model {
    ...
    function get($id, $skip_checks = "nnn"){
        $account = $this->db->query("SELECT * FROM bank WHERE bank_id=? LIMIT 1", array($id))->row_array();
        //pending balance
        ...
        if($skip_checks=="NoChecks")
        {
            $checks = $this->db->query("SELECT bc.*,DATE_FORMAT(bc.bank_checks_date, '%Y/%m/%d') AS bank_checks_date FROM bank_checks bc WHERE (join_bank_id=?) ORDER BY bank_checks_date DESC", array($account['bank_id']))->result_array();
            foreach((array)$checks AS $i => $check){
                if($check['bank_checks_status'] == "Pending" && $check['join_bank_id'] == $account['bank_id']){                        
    ...
    /* datatable related functions */
        ...
        {            
            ...
            if(isset($postData['order'])){
                ...
                public function getChecksList($bank_id,$bankType,$txnType,$postData)
                {
                    $this->get_checks_list_query($bank_id,$bankType,$txnType,$postData);
                    if($postData['length'] != -1){
                        $this->db->limit($postData['length'],$postData['start']);
                    }
                    $query = $this->db->get();
                ...
                public function countAllChecks($bank_id,$bankType,$txnType,$postData)
                {
                    $this->get_checks_list_query($bank_id,$bankType,$txnType,$postData);
                    return $this->db->count_all_results();
                }
                
                public function countAllFilteredChecks($bank_id,$bankType,$txnType,$postData)
                {
                    $this->get_checks_list_query($bank_id,$bankType,$txnType,$postData);
                    return $this->db->count_all_results();
                }
                public function get_checks_list_query($bank_id,$bankType,$txnType,$postData){
                        ...
                        {                                             
                            $this->db->where('bank_checks_to_id',$bank_id);
                            $this->db->where('bank_checks_status','Pending');
                        }         
                        //savings               
                        ...
                        $i = $_POST['start'];
                        foreach($this->checks_column_search as $item){
                            if($postData['search']['value']){
                                if($i==0){                                    
                                    $this->db->group_start();                                    
                                    $this->db->like($item, $postData['search']['value']);
                                ...
                                if(count($this->checks_column_search) - 1 == $i){
                                    $this->db->group_end();
                                }
                            }
                            $i++;
                        ...
                        if(isset($postData['order'])){
                            $this->db->order_by($this->checks_column_order[$postData['order']['0']['column']],$postData['order']['0']['dir']);
                        }else if(isset($this->checks_order)){
                            $order = $this->checks_order;
                            $this->db->order_by(key($order),$order[key($order)]);
                        }                        
                }

	...
	public static function update($player, $horse, $data, $allowed){
		...
		if($player['privileges']['privileges_horses']){ //admin can update everything else
			...
			/*  */
					...
					if( !empty($data['horses_sire']) )
					{
						if( !ctype_digit($data['horses_sire']) )
						{
								$errors['horses_sire']="Enter sire ID.";
						}elseif(!$sire || $sire['horses_gender'] != 'Stallion')
						{
							$errors['horses_sire'] = "This Sire Doesn't exist.";
						}
					}
					if($data['horses_registration_type'] == "creation" && $sire['horses_gender'] == "Gelding")
					{
						$errors['horses_sire'] = "Gelding Horse does not make Sire Please Provide Valid Sire Horse Entity.";
					}
					if( !empty($data['horses_dam']) )
					{
						if( !ctype_digit($data['horses_dam']) )
						{
		...
		/* check horse is foal year born  */
			...
			{										
				$year_exists = $CI->db->query("SELECT horses_id FROM horses WHERE  horses_birthyear = ? AND horses_id <>  ? AND horses_dam = ? LIMIT 1", array($horse['horses_birthyear'],$horse['horses_id'],$horse['horses_dam']))->row_array();			
				if($year_exists){
					$errors['horses_dam'] = "The Mare has a foal born that year.";
				}
			}
		/* check horse is foal year born  end*/
			...
			{				
				$year_exists = $CI->db->query("SELECT horses_id FROM horses WHERE  horses_birthyear = ? AND horses_id != ? AND horses_dam = ? LIMIT 1", array($data['horses_birthyear'],$horse['horses_id'],$data['horses_dam']))->row_array();
				if($year_exists){
					$errors['horses_dam'] = "The Mare has a foal born that year.";
				}
			}
...
//pre($data);exit;
	...
	public static function admin_accept_breeding($post, $player, $allowed){
		...
		$horse['horses_name'] = date('Y') . ' Foal (#' . generateId($stallion['horses_id']) .' x #' . generateId($mare['horses_id']) . ')';
	...
	public static function admin_reject_breeding($post, $player){
		...
		$message = "A moderator has rejected this breeding: <a href='/horses/view/" . $data['join_mares_id'] ."'>" . $mare['horses_competition_title'] ." " . $mare['horses_breeding_title'] . " " . $mare['horses_name'] . " #". generateId($mare['horses_id']) . "</a> x <a href='/horses/view/" . $data['join_horses_id'] ."'>" . $stallion['horses_competition_title'] ." " . $stallion['horses_breeding_title'] . " " . $stallion['horses_name'] . " #". generateId($stallion['horses_id']) . "</a> for <font color=green>$" . $data['horses_breedings_fee'] . "</font>.";
...
//pre($requests);
	...
	public static function reject_breed_request($player, $horse, $data){
		...
		$message = "Breeding request rejected. <a href='/horses/view/" . $data['join_mares_id'] ."'>" . $mare['horses_competition_title'] ." " . $mare['horses_breeding_title'] . " " . $mare['horses_name'] . " #". generateId($mare['horses_id']) . "</a> x <a href='/horses/view/" . $data['join_horses_id'] ."'>" . $stallion['horses_competition_title'] ." " . $stallion['horses_breeding_title'] . " " . $stallion['horses_name'] . " #". generateId($stallion['horses_id']) . "</a>";
